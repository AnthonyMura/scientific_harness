...
> Progress (September 2026): M0–M3 done. M3 items: SyncTeX both directions (`.scratch/module-workbench/issues/08-synctex.md`), auto-compile on save (`.scratch/module-workbench/issues/09-auto-compile-on-save.md`), image preview in the tree (`.scratch/module-workbench/issues/10-image-preview-in-tree.md`) and the compile target selector with per-target install hints (`.scratch/module-workbench/issues/11-compile-target-selector.md`). Shipped beyond the original plan: the VSCode-style split-tree layout (`.scratch/module-workbench/issues/05-vscode-style-layout.md`) and the in-app TinyTeX (`.scratch/module-workbench/issues/06-in-app-tex.md`). Since M3: interactive layout freedom — live drag reflow, four-edge splits, draggable activity-bar modules into empty panes, and the quote background for the all-windows-closed state (`.scratch/module-workbench/issues/12-interactive-layout-freedom.md`).
