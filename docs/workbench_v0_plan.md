...
- **M3 polish.** Reverse SyncTeX (PDF to source), auto-compile on save, recent projects, image preview in the tree, compile target selector UI with per-target install hints. Acceptance: the full definition of done from section 1. — **done**
- **M4 remote.** ssh target: key-based auth, project sync up, compile on the remote machine, pull back PDF + synctex + log. Acceptance: compile a project against a machine on the LAN; artifacts appear in the app and sync works against the remote paths. — **done**

> Progress (September 2026): M0–M3 done. M3 items: SyncTeX both directions (`.scratch/module-workbench/issues/08-synctex.md`), auto-compile on save (`.scratch/module-workbench/issues/09-auto-compile-on-save.md`), image preview in the tree (`.scratch/module-workbench/issues/10-image-preview-in-tree.md`) and the compile target selector with per-target install hints (`.scratch/module-workbench/issues/11-compile-target-selector.md`). Shipped beyond the original plan: the VSCode-style split-tree layout (`.scratch/module-workbench/issues/05-vscode-style-layout.md`) and the in-app TinyTeX (`.scratch/module-workbench/issues/06-in-app-tex.md`). Since M3: interactive layout freedom — live drag reflow, four-edge splits, draggable activity-bar modules into empty panes, and the quote background for the all-windows-closed state (`.scratch/module-workbench/issues/12-interactive-layout-freedom.md`). **M4 done:** ssh compile target (`.scratch/module-workbench/issues/13-ssh-compile-target.md`) — key-based auth, tar sync up, remote latexmk, PDF + synctex + log pulled back with `Input:` paths rewritten to the local root; verified end-to-end against a localhost paramiko SSH stand-in (no sudo in WSL for a real sshd). Since M4: editor cursor settings — the brick palette cursor (CodeMirror's own `.cm-cursor` via `drawSelection()`), cursor type line/block/underline, smooth-motion toggle, and a line-width sub-setting, all live from the editor gear menu (`.scratch/module-workbench/issues/14-editor-cursor.md`) — and the PDF pane opens to the right of the active editor when a compile starts (`.scratch/module-workbench/issues/15-pdf-pane-right-of-editor.md`). Since M4 (continued): the LaTeX controls moved to where the work happens — Compile with a "what to compile" picker in the editor header for .tex files, an Overleaf-style project settings menu (main file chosen from the file list, compile target, auto-compile) on the top bar, and Save version / Save As in the PDF pane that persist compiled output into the project's `versions/` folder (`.scratch/module-workbench/issues/16-latex-controls-in-editor.md`, `.scratch/module-workbench/issues/17-project-settings-main-file.md`, `.scratch/module-workbench/issues/18-pdf-save-version.md`). Compile now saves first: clicking Compile persists all open dirty editor tabs before the build starts, so an unsaved edit is no longer compiled away (`.scratch/module-workbench/issues/19-compile-saves-before-build.md`). Since 19: the editor autosaves about a second after typing stops, and closing a tab or the window flushes whatever is still pending — keepalive writes survive page teardown, so an edit is never lost to a closed tab or window (`.scratch/module-workbench/issues/20-autosave-and-save-on-close.md`). Since 20: opening a project (Recent list, Open…, New…) shows only the Explorer — no file is opened automatically; files land in the editor when picked from the tree, and a single click opens them while no editor tab is open (`.scratch/module-workbench/issues/22-open-project-explorer-only.md`). Since M4 (continued): an Overleaf-style LaTeX autocomplete overlay for `.tex` files (`.scratch/module-workbench/issues/23-latex-autocomplete.md`) — curated command list with fuzzy-ranked sections, two-step `\begin{…}` → environment picker that inserts the full paired block with the caret on the indented middle line, open-environment-first `\end` completion, and selection via mouse / arrows + Enter / Tab; verified 16/16 by a headless Chrome CDP suite. Since 23: editor spell check (`.scratch/module-workbench/issues/24-editor-spellcheck.md`) — in-browser Hunspell (hunspell-wasm) underlines misspelled words with a brick wavy line, LaTeX commands skipped; on/off toggle + dictionary select for the ten most popular languages (English first, Russian second) in the editor gear menu, dictionaries fetched from jsDelivr once and cached in IndexedDB; verified end-to-end by headless Chrome CDP (EN/RU flag sets, toggle off/on, persistence). Since 24: citation key highlight in the LaTeX regime — author names inside `\citep{…}` & kin (`\citet`, starred variants, optional prenotes/postnotes skipped) render in warm gold-bright (bone) in the editor (custom `citation` token in `latexMode.ts` + Vesper rule), verified by a tokenizer harness over styled ranges. Since 25: BibTeX support for `.bib`/`.rbib` files (`.scratch/module-workbench/issues/28-bib-editor-highlight-format.md`) — highlighting on the Vesper anchors (entry type gold-bright semibold, keys gold-bright, field names sand, comments brown italic, braces taupe) and a Format action in the editor header that pretty-prints canonically (parse → re-emit: one field per line, 2-space indent, value whitespace collapsed, comments preserved, idempotent); verified by Node tokenizer + format harnesses and headless Chrome CDP (computed token colors, Format canonicalizes the file on disk, second click a no-op). Since 28: citation-key autocomplete inside `\citep{…}` & kin (`.scratch/module-workbench/issues/29-cite-autocomplete.md`) — a References section in the completion overlay lists every key from the project's .bib files with author/year hints, filters as typed, and re-queries after each comma so multi-key citations complete in one flow; backed by `GET /api/files/bib` and a module-level bib index that reloads on project open and after every bib save; verified 22/22 by headless Chrome CDP. Since 29: PDF citation jump (`.scratch/module-workbench/issues/26-pdf-citation-jump.md`) — clicking a numbered citation in the rendered PDF smooth-scrolls to its reference entry (the bibliography is indexed from the same pdf.js text layer, so entry labels are never marked as citations), flashes it in sand, and raises a back pill that restores the exact clicked position; plain clicks still inverse-search to the source line. Pure index logic lives in `web/src/modules/pdfCitations.ts` (line grouping, citation matching with range expansion, bibliography scan); verified 32/32 by a Node harness and 11/11 by headless Chrome CDP against a natbib fixture. Since 26: PDF citation hover (`.scratch/module-workbench/issues/27-pdf-citation-hover.md`) — hovering a numbered citation raises a small tooltip with the reference's title, authors and DOI (when known); compiled mode reads the project's own `.workbench/build/<stem>.aux` (the real natbib nested `\bibcite{key}{{n}{year}{label}}` form) plus the `.bib` entries, falling back to the generated `.bbl`, then to the PDF's printed References text; static files parse the PDF's own References section. Data absence degrades silently to the raw entry text — no status chip. Pure parsing/assembly lives in `web/src/modules/pdfRefs.ts`; verified 30/30 by a Node harness and 14/14 by headless Chrome CDP against the natbib fixture (compiled + static modes). Since 27: real-project verification — the tooltip verified end-to-end on a large MDPI/natbib manuscript (95 refs; structured title/authors/DOI from the project's own aux+bib), which exposed that the dev server had been serving a stale pre-issue-27 transform after the watcher missed file changes on this share; Vite now polls for changes (`usePolling`) so stale transforms cannot persist (`.scratch/module-workbench/issues/32-vite-stale-transform.md`). A latent static-mode gap for `N.`-labeled bibliographies (MDPI print style) is tracked separately (`.scratch/module-workbench/issues/33-mdpi-static-refs-fallback.md`). Since 27 (group fix): the tooltip now lists every reference of a group citation — `[2,3]` shows both, wide ranges capped at six with a '… plus N more' line — verified on the real project's `[3,4,5]` (`huang_global_2023, liu_global_2025, liang_global_2025`) (`.scratch/module-workbench/issues/34-group-citation-tooltip.md`). Since 27 (interactive): the tooltip card is now interactive — DOIs render as clickable doi.org links (new tab), its text can be selected and copied while hovering, and a 200 ms grace period bridges marker→card so the card stays alive under the pointer; verified end-to-end on the real project including a real link click (`.scratch/module-workbench/issues/35-interactive-citation-tooltip.md`). Since 27 (static): the card is now static — placed once, bottom-right of the cursor, at show time and never tracked again, so it can be rested on for selection/copy and DOI clicks without running away (`.scratch/module-workbench/issues/36-static-citation-tooltip.md`). Since 28 (robustness fix): the BibTeX tokenizer no longer desyncs on real Zotero exports — a `%` inside a multi-line braced value is literal text, not a comment to end of line (commenting used to swallow the value's closing brace, so depth tracking stuck and every later entry rendered as untagged prose with stray taupe braces); an `@` header found where a field is expected resyncs to top level, and unterminated quoted values continue across lines; verified on a 1658-line real manuscript bibliography (all 95 entries' headers/keys/fields tagged) plus headless Chrome CDP computed colors in the live app (`workbench/web/src/bibMode.ts`, `.scratch/module-workbench/issues/28-bib-editor-highlight-format.md`). And paragraph position tags — above the first line of each .tex text paragraph, a low-contrast taupe marker like `S1·SS1·P2·L24` with bold section/subsection/paragraph counters + line number and a faint highlight on the tagged line, overlay-only, toggleable from the editor gear menu (`.scratch/module-workbench/issues/30-tex-paragraph-position-tags.md`). Since 30: toggleable line numbers (default on) with the active line's number set off in a slightly lighter cell, hairline-separated from the text - CodeMirror `lineNumbers()` + `highlightActiveLine()`/`highlightActiveLineGutter()` behind a gear-menu switch, Vesper anchors only, verified 15/15 by headless Chrome CDP (`.scratch/module-workbench/issues/31-line-numbers-active-line.md`). Since 31: the **Structure** module — a sidebar outline next to Explorer that follows focus (LaTeX sections/subsections/subsubsections/paragraphs with #30-consistent S/SS tags, Markdown ATX headings, or the displayed PDF's bookmarks via pdf.js `getOutline`), rows jump the editor cursor or scroll the PDF, live re-parse on edit and after PDF changes (`.scratch/module-workbench/issues/38-structure-module.md`).
