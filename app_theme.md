...
- Syntax (LaTeX): commands `#B38F6F` · section-level commands (`\section`, `\begin{document}`) `#C3A893` semibold · braces/arguments taupe · comments brown italic · math `$…$` pearl at 75%.
...
- **`src/vesperTheme.ts`** — CodeMirror `HighlightStyle`: commands sand
  ...
  gold-bright `#C3A893` semibold, comments brown `#8D7564` italic, delimiters
  taupe `#BCB1A0`.
- **`src/latexMode.ts`** — emits the `keyword.special` token for structural
  commands so the gold-bright rule applies.
